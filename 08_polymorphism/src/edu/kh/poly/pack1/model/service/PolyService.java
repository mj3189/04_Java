...
public class PolyService {
	 ...
	 * 		-> 조건 : 메서드명 동일, 매개 변수가 개수, 타입, 순서 하나라도 달라야 함
	...
	public void method1() {
		...
		// -> 자식 객체 내부에 존재하는 부모 타입(SmartPhone)객체만 참조하게 된다!
...
//		s2.setAnroidVersion(15);  // 오류
	 ...
	 * 	부모 타입 잠조 변수를 자식 타입으로 강제 형변환 하여
	 * 	다시 자식 객체를 참조하도록 변경(부모 -> 자식) 업캐스팅이 존재해야 다운캐스팅이 가능
	 * 
	 */
	public void method2() {
...
//		int v2 = s2.getAndroidVersion();
	...
	public void method3() {
		...
		// Galazy 클래스의 setAndroidVersion(); 메서드와 연결
	...
	public void method5() {
	  ...
	  // 부모 참조 변수 = 자식 객체 (업캐스팅 상태)
		...
		for(SmartPhone s : arr){  // [1],[2] 인덱스에서 업캐스팅 적용
			// 매 반복 시 마다 arr의 욧소를 하나씩 꺼내 s에 저장
	...
	public void printToString(SmartPhone s) {
		...
		// 실제 참조하는 객체가 자식 객체면
		// 자식 객체의 오버바이딩된 toString()호출 (동적 바인딩)
		System.out.println(s.toString());
...
//	}
	...
	public void method7() {
		...
		printToString(createPhone(3));   // net Galaxy()으로 생성된 객체 주소
	...
	public SmartPhone createPhone(int num) {
		...
		return new Galaxy();   // 오류 안나는 이유 : 반환형에 업캐스팅 적용
